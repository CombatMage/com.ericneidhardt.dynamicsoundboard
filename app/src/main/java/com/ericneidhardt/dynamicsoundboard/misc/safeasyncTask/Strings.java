package com.ericneidhardt.dynamicsoundboard.misc.safeasyncTask;
...
public class Strings
{
	...
	public static String capitalize(String s)
	{
		...
		return c.length() >= 2 ? c.substring(0, 1).toUpperCase(Locale.ENGLISH) + c.substring(1)
				: c.length() >= 1 ? c.toUpperCase(Locale.ENGLISH)
						: c;
	}
	...
	{
		return Strings.toString(a).toLowerCase(Locale.ENGLISH).equals(Strings.toString(b).toLowerCase());
	}
	...
	public static String[] chunk(String str, int chunkSize)
	{
		...
		for (int i = 0; i < arrayLen; ++i)
			array[i] = str.substring(i * chunkSize, (i * chunkSize) + chunkSize < len ? (i * chunkSize) + chunkSize
					: len);
